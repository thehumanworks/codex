...
class EdgeLauncherTests(unittest.TestCase):
    ...
    def test_literal_arguments_are_not_shell_expanded_or_modified(self):
        ...
        self.assertEqual(arguments, ["exec", "$(touch should-not-exist); $TOKEN", "--json"])
    ...
    def test_exec_preserves_native_process_arguments(self):
        ...
        execute.assert_called_once_with(str(self.binary), command(self.binary, arguments))
    ...
    def test_existing_security_and_model_overrides_are_forwarded(self):
        arguments = ["--sandbox", "read-only", "--ask-for-approval", "on-request", "--model", "example-model", "--disable", "agent_message_board"]
        with patch("edge.os.execv") as execute:
            main(["--codex-bin", str(self.binary), "--", *arguments])
        self.assertEqual(execute.call_args.args[1][-len(arguments):], arguments)

    def test_environment_can_select_the_fork_binary(self):
        with patch.dict(os.environ, {"CODEX_EDGE_BIN": str(self.binary)}), patch("edge.os.execv") as execute:
            main(["--", "--help"])
        self.assertEqual(execute.call_args.args[0], str(self.binary))

    def test_missing_binary_fails_before_execution(self):
        with patch("edge.os.execv") as execute, contextlib.redirect_stderr(io.StringIO()), self.assertRaises(SystemExit) as error:
    ...
    def test_launcher_does_not_log_user_arguments_or_environment(self):
        ...
        with patch("edge.os.execv"), contextlib.redirect_stdout(output), contextlib.redirect_stderr(errors):
            main(["--codex-bin", str(self.binary), "--", "exec", "sensitive synthetic input"])

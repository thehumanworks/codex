...
def main():
    ...
    parser.add_argument("stage", choices=("source-auth", "packaged-auth", "workspace", "combined"))
    ...
    subprocess.run(["git", "diff", "--exit-code", tested, "HEAD", "--", "codex-rs", "scripts", "superfork/edge.py", "superfork/test_edge.py", "superfork/verify_edge_cli.py", "superfork/complete_verification.py"], check=True)
    result = {"status": "stage-passed-not-default-branch-delivery", "stage": args.stage, "tested_commit": tested, "runtime_code_tree": code_tree, "workflow_run": os.environ["GITHUB_RUN_ID"], "platform": "Linux x86_64"}
    if args.stage in ("source-auth", "packaged-auth"):
        result["authentication"] = unittest_summary(evidence / f"{args.stage}.log", 17)
        result["auth_test_sha256"] = hashlib.sha256(Path("scripts/test_chatgpt_env_auth.py").read_bytes()).hexdigest()
        if args.stage == "packaged-auth":
            output = (evidence / "edge-native.log").read_text()
            if "PASS: 2 native CLI controls, with explicit user-disable precedence" not in output:
                raise ValueError("Packaged native edge controls did not pass")
            result["edge_native_controls"] = output
            result["package_manifest"] = json.loads((evidence / "package-manifest.json").read_text())
            result["package_binaries"] = (evidence / "package-binaries.sha256").read_text()
    elif args.stage == "workspace":
        result["workspace"] = nextest_summary(evidence / "workspace.log")
        result["commands"] = ["cargo build --locked --workspace --bins", "just test --locked"]
    else:
        stages = {}
        for name in ("source-auth", "packaged-auth", "workspace"):
            data = json.loads(Path(f"superfork/evidence/completion-{name}.json").read_text())
            if any(data[key] != result[key] for key in ("tested_commit", "runtime_code_tree", "workflow_run")):
        ...
        for name in ("final-acceptance", "final-affected"):
            ...
            if data["tested_code_tree"] != code_tree or data["status"] != "stage-passed-not-default-branch-delivery":
        ...
        for name, expected in edge["source_sha256"].items():
            if hashlib.sha256(Path("superfork", name).read_bytes()).hexdigest() != expected:

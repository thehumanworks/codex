...
def nextest_summary(path):
    ...
    return {
        "summary": lines[0],
        "tests_run": int(counts[1]),
        "retries": [
            line.strip()
            for line in text.splitlines()
            if "RETRY" in line or "FLAKY" in line
        ],
    }
...
def main():
    ...
    parser.add_argument(
        "stage", choices=("acceptance", "affected", "smoke", "workspace")
    )
    ...
    subprocess.run(
        ["git", "diff", "--exit-code", source, "HEAD", "--", "codex-rs", "scripts"],
        check=True,
    )
    result = {
        "stage": args.stage,
    ...
    if args.stage == "acceptance":
        ...
        result["repetitions"] = [
            nextest_summary(evidence / f"concurrency-{n}.log") for n in range(1, 6)
        ]
        ...
        if re.search(r"Ran [1-9][0-9]* tests? in ", text) is None or not re.search(
            r"^OK$", text, re.MULTILINE
        ):
            raise ValueError(
                "Authentication smoke suite did not finish successfully without skips"
            )
        ...
        for name in ("acceptance", "affected", "smoke"):
            previous = json.loads(
                Path(f"superfork/evidence/final-{name}.json").read_text()
            )
            if (
                previous["workflow_run"] != result["workflow_run"]
                or previous["tested_code_tree"] != tree
            ):
                raise ValueError(
                    "Stage evidence belongs to a different run or runtime tree"
                )
            stages[name] = previous
        result["stages"] = stages
        result["limitations"] = [
            "Only Linux x86_64 was executed; macOS/Windows are not verified.",
            "Injected store durability is the host implementation's responsibility.",
            "Lossless delivery is opt-in for the in-process API, not websocket/stdio or disk replay.",
            "No live paid inference or cross-host/cloud-store implementation is validated here.",
        ]

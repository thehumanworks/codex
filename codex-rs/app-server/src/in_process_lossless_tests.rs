...
async fn warning_runtime(
    ...
    let (home, mut args) =
        tests::build_test_start_args(SessionSource::Cli, /*channel_capacity*/ 1).await;
...
async fn collect_shutdown_warnings(mut client: InProcessClientHandle) -> Vec<String> {
    let shutdown = timeout(SHUTDOWN_TIMEOUT, client.begin_shutdown())
        .await
        .expect("shutdown admission must not block on data queues")
        .expect("shutdown should be admitted");
    let mut warnings = Vec::new();
    while let Some(event) = timeout_at(shutdown.deadline, client.next_event())
        .await
        .expect("event stream should close within the shutdown deadline")
    {
        if let InProcessServerEvent::ServerNotification(notification) = event
            && let ServerNotification::ConfigWarning(warning) = *notification
        {
...
async fn in_process_lossless_drains_accepted_store_request_and_preserves_event_order() {
    ...
    let response = response_rx
        .await
        .expect("accepted request must receive a result")
...
async fn in_process_best_effort_retains_ordinary_notification_drop_policy() {
    ...
    client
        ...
        .await
        .expect("control request transport")
...
async fn in_process_lossless_rejects_duplicate_shutdown_without_unbounded_admission() {
    ...
    let shutdown = client
        .begin_shutdown()
        .await
        .expect("first shutdown accepted");
    assert_eq!(
        client
            .begin_shutdown()
            .await
            .expect_err("second shutdown rejected")
            .kind(),
...
async fn in_process_request_cancellation_survives_the_transport_changes() {
    ...
    let Some(InProcessClientMessage::Request {
        cancellation,
        response_tx,
        ..
    ...
    assert!(
        request
            .await
            .expect_err("request future should be cancelled")
            .is_cancelled()
    );
    assert!(cancellation.is_cancelled());
    drop(response_tx);
...
async fn in_process_command_overload_remains_explicit() {
    ...
    .await
    .expect("overload must not block")
    .expect_err("saturated queue rejects admission");

...
async fn in_process_injected_store_survives_runtime_restart_without_ambient_queue() {
    let store = Arc::new(InMemoryThreadStore::default());
    let (_first_home, mut first_args) = tests::build_test_start_args(
        SessionSource::Cli,
        DEFAULT_IN_PROCESS_CHANNEL_CAPACITY,
    )
    .await;
    ...
    let created: ThreadStartResponse = serde_json::from_value(created).expect("typed start response");
    ...
    assert_eq!(queue_error, invalid_request("user message queue is unavailable"));
    client.shutdown().await.expect("first runtime should shut down");

    let (_second_home, second_args) = tests::build_test_start_args(
        SessionSource::Cli,
        DEFAULT_IN_PROCESS_CHANNEL_CAPACITY,
    )
    .await;
    ...
    client.shutdown().await.expect("second runtime should shut down");
}

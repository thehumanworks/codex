...
async fn in_process_shutdown_drains_multiple_accepted_rpcs_in_both_delivery_modes() {
    for delivery in [
        ...
        let (_home, args) =
            tests::build_test_start_args(SessionSource::Cli, /*channel_capacity*/ 4).await;
        ...
        client
            .shutdown()
            .await
            .expect("accepted work drains gracefully");
        for response in responses {
            let value = response
                .await
                .expect("response sender survives")
                .expect("list RPC completes");
            let result: ThreadListResponse =
                serde_json::from_value(value).expect("typed list response");
...
async fn in_process_shutdown_rejects_late_requests_on_retained_senders() {
    ...
    while timeout_at(shutdown.deadline, client.next_event())
        .await
        .expect("event drain remains bounded")
        .is_some()
    {}
    let error = sender
        ...
        .await
        .expect_err("retained sender cannot reopen admission");
    assert_eq!(error.kind(), ErrorKind::BrokenPipe);
    client
        .finish_shutdown(shutdown)
        .await
        .expect("retained sender does not stall shutdown");
}
